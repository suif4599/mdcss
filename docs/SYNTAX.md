...
默认策略为 `auto`（需 `--enable-parser`）：条纹按「条带」着色，跨行合并覆盖的所有行与其锚点行强制归入同一条带，合并单元格整块显示同一颜色，不会出现同色连缀或条纹断裂；多列交错合并时条带自动传递合并。条带自首个数据行起交替着色（第 1 条带浅、第 2 条带深），表格无合并单元格时与逐行交替完全一致。

可在表格标题行的 `Table` 与 `:` 之间加标签切换策略（见「表格标题」一节，需启用 parser）：`Table<auto>: 标题` 为默认值（不加标签即等同）；`Table<zebra>: 标题` 严格按源码行号逐行交替，合并块取锚点行颜色；`Table<nozebra>: 标题` 禁用斑马纹（表头底色保留）。
