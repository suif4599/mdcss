...
// plumbing: pre(markdown) -> markdown, post(html) -> html, plus the line-shift
// ledger state that MPE keeps alive on globalThis between the two hooks.
// The html input is a fixture baked from a real markdown-it (html:true)
// capture — MPE's own crossnote renderer cannot be imported here; test_md/
// is the manual crosscheck against the real preview.
...
process.stdout.write(JSON.stringify({ markdown, html, shifts }));
